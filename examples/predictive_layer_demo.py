...
def demo_batch_prediction(predictor):
    ...
    print(f"✓ Actions for each scenario:")
    
    for name, action in zip(scenario_names, actions):
        ...
        print(f"  - {name:12}: Battery: {battery_desc:9} ({battery_action:+.3f}), "
...
def demo_grid_integration():
    ...
    print(f"✓ Control predictor loaded")
    
    # Simulate grid operation with RL control
    print(f"✓ Running grid simulation with RL control...")
    ...
    for step in range(5):
        ...
        print(f"  Step {step+1}: Freq={new_state.frequency_hz:.2f}Hz, "
...
def demo_convenience_functions():
    ...
    print(f"✓ Convenience functions provide simple API for common use cases")
