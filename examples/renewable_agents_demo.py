...
class RenewableAgentsDemo:
    ...
    def _create_demo_agents(self):
        ...
        for i in range(3):
            ...
            solar_agent = SolarAgent(
                solar_panel=solar_panel, agent_id=f"solar_agent_{i}", communication_range=3.0, coordination_weight=0.3
            )
        ...
        for i in range(3):
            ...
            wind_agent = WindAgent(
                wind_turbine=wind_turbine, agent_id=f"wind_agent_{i}", communication_range=3.0, coordination_weight=0.3
            )
    ...
    def _calculate_demand_response(self) -> list[DemandResponseResult]:
        ...
        for agent in self.agents:
            if isinstance(agent, SolarAgent):
                # Solar forecasts
                forecast_irradiance = [max(0, 1000 * np.sin(np.pi * (self.time_step + h) / 12)) for h in forecast_times]
                forecast_prices = [60.0 + 40.0 * (0.7 + 0.3 * np.sin(2 * np.pi * h / 24)) for h in forecast_times]
                forecast_grid_stress = [abs(0.1 * np.sin(2 * np.pi * h / 24)) for h in forecast_times]
                ...
                # Wind forecasts
                forecast_wind_speed = [
                    10.0 * (1.0 + 0.3 * np.sin(2 * np.pi * (self.time_step + h) / 24)) for h in forecast_times
                ]
                forecast_prices = [60.0 + 40.0 * (0.7 + 0.3 * np.sin(2 * np.pi * h / 24)) for h in forecast_times]
                forecast_grid_stress = [abs(0.1 * np.sin(2 * np.pi * h / 24)) for h in forecast_times]
    ...
    def print_summary(self):
        ...
        print(
            f"Total Capacity: {sum(a.capacity_mw for a in [ag.solar_panel for ag in solar_agents] + [ag.wind_turbine for ag in wind_agents]):.1f} MW"
        )
