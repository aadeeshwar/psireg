...
from psireg.sim.assets import Battery, Load, SolarPanel, WindTurbine
...
def create_renewable_grid_simulation():
    ...
    simulation_config = SimulationConfig(timestep_minutes=15, horizon_hours=48, max_assets=100)

    grid_config = GridConfig(frequency_hz=60.0, voltage_kv=230.0, stability_threshold=0.1, max_power_mw=1000.0)
    ...
    generation_node = NetworkNode(
        node_id="generation_hub", name="Generation Hub", voltage_kv=138.0, x_coordinate=0.0, y_coordinate=0.0
    )

    load_node = NetworkNode(
        node_id="load_center", name="Load Center", voltage_kv=138.0, x_coordinate=5.0, y_coordinate=0.0
    )

    storage_node = NetworkNode(
        node_id="storage_facility", name="Storage Facility", voltage_kv=138.0, x_coordinate=2.5, y_coordinate=0.0
    )
...
def simulate_grid_operations(engine, agents, duration_hours=48):
    ...
    for step in range(total_steps):
        ...
        if step % 20 == 0:
            ...
            print(
                f"  Progress: {progress:.1f}% - Time: {current_time.strftime('%Y-%m-%d %H:%M')} - "
...
def analyze_simulation_results(metrics_data):
    ...
    max_generation = max(
        [s + w for s, w in zip(metrics_data["solar_output_mw"], metrics_data["wind_output_mw"], strict=False)]
    )
