...
def setup_demo_grid() -> GridEngine:
    ...
    city_load = Load(
        asset_id="city_load_1", name="City Load", node_id="load_center", capacity_mw=300.0, baseline_demand_mw=220.0
    )
    ...
    for step in range(5):
        ...
        for asset in controllable_assets:
            if isinstance(asset, Battery):
                ...
                observation.extend([asset.current_demand_mw / asset.capacity_mw, 1.0])  # Load factor placeholder
        ...
        print(
            f"  Reward: {reward:.3f} (freq_dev: {abs(new_state.frequency_hz - 60.0):.3f}, balance: {abs(new_state.power_balance_mw):.1f})"
        )
...
def demonstrate_ppo_training():
    ...
    try:
        ...
        print("Trainer created with config:")
