...
def demonstrate_demand_response():
    ...
    for price in test_prices:
        ...
        print(
            f"   Price: ${price:.0f}/MWh ({price_change:+.0f}%) → Demand: {responsive_demand:.1f} MW ({demand_change:+.1f}%)"
        )
...
def demonstrate_swarm_coordination():
    ...
    for i, agent in enumerate(agents):
        load = agent.load
        print(
            f"   {load.name}: {load.baseline_demand_mw:.0f} MW baseline, "
    ...
    print(
        f"   Coordination effectiveness: {total_reduction/(sum(load.dr_capability_mw for load in loads))*100:.1f}% of total DR capability"
    )
