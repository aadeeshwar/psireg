"""Tests for WindAgent swarm intelligence coordination.

This module provides comprehensive tests for the WindAgent class that combines
...
class TestWindAgentCore:
    """Test WindAgent core functionality."""

    def test_wind_agent_initialization(self):
        """Test WindAgent initialization."""
        # Create wind turbine
        ...
        # Import WindAgent (will be implemented)
        from psireg.swarm.agents.wind_agent import WindAgent

        # Create agent
        agent = WindAgent(
            wind_turbine=wind_turbine,
            agent_id="wind_agent_001",
        ...
        assert agent.wind_turbine == wind_turbine
        assert agent.agent_id == "wind_agent_001"
    ...
    def test_wind_agent_default_agent_id(self):
        """Test WindAgent with default agent ID."""
        wind_turbine = WindTurbine(
            asset_id="wind_002",
        ...
        agent = WindAgent(wind_turbine=wind_turbine)

        # Should use wind turbine asset_id as default
        assert agent.agent_id == "wind_002"

    def test_wind_agent_state_management(self):
        """Test WindAgent state management."""
        wind_turbine = WindTurbine(
            asset_id="wind_003",
            name="Test Wind Turbine 3",
            node_id="wind_node",
            capacity_mw=75.0,
        )
        wind_turbine.set_status(AssetStatus.ONLINE)

        from psireg.swarm.agents.wind_agent import WindAgent, WindSwarmState
        ...
        assert isinstance(state, WindSwarmState)
        assert state.agent_id == "wind_003"
        assert state.current_generation_mw == 0.0  # Initial generation
        assert state.capacity_mw == 75.0
        assert state.rotor_diameter_m == 150.0  # Default rotor diameter
...
class TestWindAgentPPOIntegration:
    """Test WindAgent PPO forecasting integration."""

    def test_ppo_predictor_integration(self):
        """Test PPO predictor integration."""
        wind_turbine = WindTurbine(
            asset_id="wind_004",
            name="Test Wind Turbine 4",
            node_id="wind_node",
            capacity_mw=100.0,
        )
    ...
    def test_generation_forecasting(self):
        ...
        assert all(0.0 <= f <= wind_turbine.capacity_mw for f in forecast)

    def test_forecast_caching(self):
        """Test forecast caching mechanism."""
        wind_turbine = WindTurbine(
            asset_id="wind_006",
            name="Test Wind Turbine 6",
            node_id="wind_node",
            capacity_mw=100.0,
        )
    ...
    def test_wind_speed_based_forecasting(self):
        """Test wind speed-based forecasting features."""
        wind_turbine = WindTurbine(
            asset_id="wind_008",
            name="Test Wind Turbine 8",
            node_id="wind_node",
            capacity_mw=100.0,
        )
        ...
        wind_speeds = [2.0, 8.0, 12.0, 20.0, 30.0]  # Below cut-in to above cut-out
        ...
        for wind_speed in wind_speeds:
            ...
            forecast = agent.forecast_generation(grid_conditions, hours=1)

            assert isinstance(forecast, list)
            assert len(forecast) == 1

            # Verify wind speed constraints
            if wind_speed < wind_turbine.cut_in_speed_ms:
                assert forecast[0] == 0.0  # Below cut-in
            elif wind_speed >= wind_turbine.cut_out_speed_ms:
                assert forecast[0] == 0.0  # Above cut-out
            else:
                assert forecast[0] >= 0.0  # Active generation range


class TestWindAgentPheromoneCoordination:
    """Test WindAgent pheromone coordination."""

    def test_grid_conditions_update(self):
        """Test grid conditions update."""
        wind_turbine = WindTurbine(
            asset_id="wind_009",
            name="Test Wind Turbine 9",
            node_id="wind_node",
            capacity_mw=100.0,
        )
        ...
        assert wind_turbine.current_wind_speed_ms == 12.0
        assert wind_turbine.current_air_density_kg_m3 == 1.225
        assert wind_turbine.current_weather_condition == WeatherCondition.WINDY

    def test_swarm_signal_update(self):
        """Test swarm signal update."""
        wind_turbine = WindTurbine(
            asset_id="wind_010",
            name="Test Wind Turbine 10",
            node_id="wind_node",
            capacity_mw=100.0,
        )
    ...
    def test_coordination_signal_generation(self):
        ...
        # Set some generation
        wind_turbine.current_output_mw = 75.0
...
class TestWindAgentDemandResponse:
    """Test WindAgent demand response generation (primary output)."""

    def test_demand_response_calculation(self):
        """Test demand response signal calculation."""
        wind_turbine = WindTurbine(
            asset_id="wind_013",
            name="Test Wind Turbine 13",
            node_id="wind_node",
            capacity_mw=100.0,
        )
        ...
        demand_response = agent.calculate_demand_response_signal(
            forecast_wind_speed=[12.0, 15.0, 10.0],
    ...
    def test_curtailment_based_demand_response(self):
        ...
        # Set high wind conditions
        wind_turbine.current_wind_speed_ms = 15.0
        wind_turbine.current_air_density_kg_m3 = 1.225
        wind_turbine.current_weather_condition = WeatherCondition.WINDY
    ...
    def test_demand_response_signal_integration(self):
        ...
        # Set wind turbine conditions for generation
        wind_turbine.set_wind_speed(12.0)
        wind_turbine.set_air_density(1.225)
        wind_turbine.set_weather_condition(WeatherCondition.WINDY)
    ...
    def test_wind_variability_demand_response(self):
        """Test demand response considering wind variability."""
        wind_turbine = WindTurbine(
            asset_id="wind_019",
            name="Test Wind Turbine 19",
            node_id="wind_node",
            capacity_mw=100.0,
        )
        ...
        # Test with highly variable wind speeds
        variable_wind_speeds = [5.0, 15.0, 8.0, 20.0, 3.0]

        demand_response = agent.calculate_demand_response_signal(
            forecast_wind_speed=variable_wind_speeds,
            forecast_prices=[50.0] * len(variable_wind_speeds),
            forecast_grid_stress=[0.3] * len(variable_wind_speeds),
        )

        # Should handle variability and provide appropriate demand response
        assert isinstance(demand_response, dict)
        assert "signal_mw" in demand_response
        assert "confidence" in demand_response

        # Lower confidence expected due to wind variability
        assert demand_response["confidence"] >= 0.0


class TestWindAgentSwarmIntegration:
    """Test WindAgent integration with swarm infrastructure."""
    ...
    def test_swarm_bus_integration(self):
        ...
        # Create wind turbine and agent
        wind_turbine = WindTurbine(
            asset_id="wind_020",
            name="Test Wind Turbine 20",
            node_id="wind_node",
            capacity_mw=100.0,
        )
    ...
    def test_multi_agent_coordination(self):
        """Test coordination between multiple wind agents."""
        ...
        for i in range(3):
            turbine = WindTurbine(
                asset_id=f"wind_022_{i}",
                name=f"Test Wind Turbine 22_{i}",
            ...
            agent = WindAgent(wind_turbine=turbine)
            agents.append(agent)

        # Simulate coordination signals
        coordination_signals = [0.2, 0.5, 0.8]

        for i, agent in enumerate(agents):
            # Update each agent with neighbor signals
            neighbor_signals = [coordination_signals[j] for j in range(3) if j != i]
            agent.update_swarm_signals(neighbor_signals)
...
class TestWindAgentPerformance:
    """Test WindAgent performance and edge cases."""

    def test_large_scale_coordination(self):
        """Test large-scale coordination performance."""
        wind_turbine = WindTurbine(
            asset_id="wind_023",
            name="Test Wind Turbine 23",
            node_id="wind_node",
            capacity_mw=100.0,
        )
    ...
    def test_extreme_weather_conditions(self):
        """Test agent behavior under extreme weather conditions."""
        wind_turbine = WindTurbine(
            asset_id="wind_024",
            name="Test Wind Turbine 24",
            node_id="wind_node",
            capacity_mw=100.0,
        )
        ...
        extreme_conditions = [
            {"wind_speed_ms": 50.0, "weather_condition": WeatherCondition.STORMY},
            {"wind_speed_ms": 1.0, "weather_condition": WeatherCondition.FOGGY},
            {"wind_speed_ms": 30.0, "weather_condition": WeatherCondition.SNOWY},
        ]
        ...
        for conditions in extreme_conditions:
            agent.update_grid_conditions(
                ...
                local_generation_mw=500.0,
                electricity_price=200.0,
            ...
            demand_response = agent.calculate_demand_response_signal(
                forecast_wind_speed=[conditions["wind_speed_ms"]] * 3,
                forecast_prices=[200.0] * 3,
                forecast_grid_stress=[1.0] * 3,
            )
    ...
    def test_string_representations(self):
        ...
        str_repr = str(agent)
        assert "WindAgent" in str_repr
        assert "wind_026" in str_repr

        # Test detailed representation
        repr_str = repr(agent)
        assert "WindAgent" in repr_str
        assert "wind_026" in repr_str
        assert "100.0" in repr_str  # Capacity

    def test_forecast_performance(self):
        """Test forecasting performance with various conditions."""
        wind_turbine = WindTurbine(
            asset_id="wind_027",
            name="Test Wind Turbine 27",
            node_id="wind_node",
            capacity_mw=100.0,
        )
        ...
        for horizon in horizons:
            ...
            forecast = agent.forecast_generation(grid_conditions, hours=horizon)
